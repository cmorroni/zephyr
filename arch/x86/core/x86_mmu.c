 ...
 */
BUILD_ASSERT(CONFIG_SRAM_BASE_ADDRESS + (CONFIG_SRAM_SIZE * 1024ULL) - 1ULL <=
...
static inline bool is_within_system_ram(uintptr_t addr)
{
	...
	return addr < (CONFIG_SRAM_BASE_ADDRESS + (CONFIG_SRAM_SIZE * 1024U));
#else
	return (addr >= CONFIG_SRAM_BASE_ADDRESS) &&
		(addr < (CONFIG_SRAM_BASE_ADDRESS + (CONFIG_SRAM_SIZE * 1024U)));
#endif
}
				...
				struct k_mem_partition *partition)
{
	...
	__ASSERT(partition->start >= CONFIG_SRAM_BASE_ADDRESS,
		 "region at %08lx[%zu] extends below system ram start 0x%08x",
		 partition->start, partition->size, CONFIG_SRAM_BASE_ADDRESS);
	__ASSERT(((partition->start + partition->size) <=
		  (CONFIG_SRAM_BASE_ADDRESS + (CONFIG_SRAM_SIZE * 1024U))),
